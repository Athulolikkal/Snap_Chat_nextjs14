...
const imagePreviewDialog:React.FC<ImagePreviewDialogProps> = ({selectedFile,onClose,onImageChange,setStep}) => {
  return (
    <Dialog open={!!selectedFile}>
        <DialogContent className='bg-sigMain border-sigColorBgBorder md:max-w-3xl max-w-xl h=[80vh] flex flex-col' onInteractOutside={onClose}>
         <DialogHeader className='flex-1'>
          <div className='flex items-center relative h-3/4 my-auto'>
            <Image src={selectedFile!} alt='selected file' fill className='rounded-md border mx-auto border-sigColorBgBorder object-contain' />
          </div>
         </DialogHeader>
         <DialogFooter className='mx-auto flex items-center'>
          ...
          <Button size={"sm"} onClick={onImageChange} className='rounded-full px-4' >
            ...
            <Button size={"sm"} onClick={() => setStep && setStep(1)} className='rounded-full bg-sigSnapChat px-4  hover:bg-sigSnapChat ' >
                Next
            </Button>
         </DialogFooter>
        </DialogContent>
